...
/* Custom jquery alerts *//*global define*/
...
(function($) {
    $.fn.drags = function(options) {
        ...
        //options = $.extend({handle:"",cursor:"move"}, opt);

        options = $.extend($.fn.drags.defaults, options);
        ...
        return $el.css('cursor', options.cursor).on("mousedown", function(e) {
            ...
            if (options.condition()) {                
                ...
                var z_idx = $drag.css('z-index'),
                    ...
                    mX = e.pageX,
                    mY = e.pageY,
                    pos_y = $drag.offset().top + drg_h - mY;
                    pos_x = $drag.offset().left + drg_w - mX;

                $drag.css('z-index', 1000).parent().on("mousemove", function(e) { 

                    mY = (options.axis == "y") ? e.pageY : mY;
                    mX = (options.axis == "X") ? e.pageY : mX;                

                    if (options.boundary != null && options.boundary != '') {                        
                        
                        var bY1 = options.boundary.offset().top;
                        var bY2 = options.boundary.offset().top + options.boundary.outerHeight();     
                        if (mY < bY1) {
                            mY = bY1;
                            $el.trigger( "mouseup" );
                        } else if (mY > bY2) {
    ...
    $.fn.drags.defaults = {
         handle:        "", // handle to apply
         cursor:       "move", // cursor to use when dragging
         axis:    null, // limit to axis (either x or y)
         condition: function() { return true; }, // function required in order to move
         move: function() { return true; }, // while dragged
         start: function() { return true; }, // at start of drag
         stop: function() { return true; }, // at end of drag
         boundary: null // a div element to use as the drag bounds
    };
